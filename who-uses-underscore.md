...
Underscore 是面向代理、应用与协议的基础设施。有些用户通过 Hightop 这样的产品感知到它，这类产品基于 Underscore 构建，把面向 AI 代理的数字银行体验做得足够简单，供人类直接使用；有些通过运行在可编程钱包中的 AI 系统感知到它；而大多数终端用户甚至不会知道它的存在。
...
[Hightop](https://hightop.com) 是面向 AI 代理的数字银行产品，同时对人类保持简单易用。让 AI 代理付款并收款，而控制权始终在用户手中。Hightop 在后台处理[可编程钱包](user-wallet.md)基础设施、Gas 和资金流转，而 Underscore 提供 AI 代理权限、支付、提现和收益自动化所需的资金控制与执行层。Hightop 的 Earn 体验同时使用核心 Vaults 与 Amplified Vaults，而借贷以及 Amplified Vault 背后的杠杆轨道由 Ripe 提供。

### Hightop 为用户提供什么

在一个产品里，Hightop 用户可以：

- **为账户注资**，并通过熟悉的入金和出金路径转移资金
- **为每个 AI 代理设置限额、权限和支付规则**
- **让 AI 代理付款、收款、赚取收益、借款、投资和转移资金**，而无需把私钥或不受限制的钱包访问权交给它们
- **在一个简单的应用里管理一切**，同时把[可编程钱包](user-wallet.md)基础设施、Gas 和资金流转隐藏在后台，并让控制层保持链上可验证

### Underscore 如何驱动这些功能

**AI 代理控制层**

- Hightop 的 AI 代理控制配置映射到 Underscore 的[管理员](managers.md)
- 钱包级与按 AI 代理划分的规则定义每个 AI 代理能做什么、能接触哪些资产和场所、能向谁付款、能花多少钱以及何时到期
- 关键边界由不可变的开源智能合约在链上强制执行，而不是只靠应用后端记住

**支付与资金流转**

- 已批准的周期性对手方映射到 Underscore 的[收款人](payees.md)
- 临时付款和大额采购使用带可选审核延迟和取消窗口的[数字支票](cheques.md)
- 可信出金与自托管路径使用[白名单](whitelist.md)，可即时转到已批准地址
- AI 代理也可以通过一次性或按使用量计费的流程收款。研究代理可以按请求收费，支持代理可以按工作流收费
- 这就是 Hightop 如何在不给 AI 代理开放式账户权限的前提下，让它们可以向供应商付款、收款并转移资金

**收益与投资组合自动化**

- Hightop 的 Earn 流程使用 Underscore 的收益 Lego，再加上[核心 Vaults](vaults.md)和[Amplified Vaults](vaults.md)，让闲置现金在多种收益档位下持续生息
...
- 借贷层由 Ripe 提供，Amplified Vault 背后的杠杆轨道同样由 Ripe 提供；而 Underscore 负责外围的 AI 代理控制、[可编程钱包](user-wallet.md)权限模型以及 AI 代理执行模型
...
3. 创建一个 AI 代理并给它清晰的规则
4. 让这个 AI 代理在自己的权限范围内付款、收款、赚取收益、投资或保护投资组合
5. 需要时借款，并随时提现到可信地址
